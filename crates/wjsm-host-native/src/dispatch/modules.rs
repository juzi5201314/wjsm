...
/// 句柄是否为登记在册的 ScopeRecord（嵌套闭包会以普通 env 对象充当环境）。
pub(crate) fn is_scope_record(state: &NativeAgentState, record: i64) -> bool {
    object_handle(record).is_some_and(|record| state.scope_records.contains_key(&record))
}
...
fn execute_module_bundle(
    ...
    state
        .install_isolated_program(ctx, image, artifact.program())
        .map_err(|error| ModuleLoadFailure::Message(error.to_string()))?;
...
fn execute_vm_program(
    ...
    state
        .install_isolated_program(ctx, image, artifact.program())
        .map_err(|error| VmExecutionError::Compile(anyhow::anyhow!(error.to_string())))?;
    state
        .activate_image(ctx, image_id)
