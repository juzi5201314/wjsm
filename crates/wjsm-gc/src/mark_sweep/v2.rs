//! memory64 V2 mark-sweep policy（后端无关）。
//!
//! 该策略只依赖 V2 allocator、atomic handle table 与显式 root snapshot；不会读取
//! main-memory `obj_table`。堆经 `M: GrowableHeapMemory` 注入。
...
use crate::heap::{
    Allocation, AllocatorError, EpochParticipant, GrowableHeapMemory, HandleGeneration, HandleId,
    HandleTableError, HandleTableV2, ManagedHeap, ManagedHeapLayout, Nlab,
};
...
pub struct MarkSweepV2<M: GrowableHeapMemory> {
    heap: ManagedHeap<M>,
...
impl<M: GrowableHeapMemory> MarkSweepV2<M> {
    pub fn new(memory: M, layout: ManagedHeapLayout) -> Result<Self, MarkSweepV2Error> {
        memory
