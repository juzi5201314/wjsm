//! Generational ZGC V2 collector（后端无关，`M: GrowableHeapMemory`）。
...
use crate::heap::{
    Allocation, GrowableHeapMemory, HandleGeneration, HandleId, HandleTableV2, HeapAddress,
    ManagedHeap, ManagedHeapLayout, Nlab,
};
...
/// ZGC V2 增量 mark/relocate collector，经 `M: GrowableHeapMemory` 单态化。
pub struct ZgcV2<M: GrowableHeapMemory> {
    heap: ManagedHeap<M>,
...
impl<M: GrowableHeapMemory> ZgcV2<M> {
    pub fn new(memory: M, layout: ManagedHeapLayout) -> Result<Self, ZgcV2Error> {
        memory
