            ...
            match export {
                ...
                ExportEntry::Default { .. } => {
                    if !collected.names.insert("default".to_string()) {
                        bail!(
                            "Duplicate export 'default' in module '{}'",
                            node.path.display()
                        );
                    }
                }
...
mod tests {
    ...
    fn produces_runtime_consumable_link_result() {
        ...
        assert_eq!(
            imports[0].names,
            vec![("localValue".to_string(), "value".to_string())]
        );
    ...
    fn wildcard_reexport_allows_any_import() {
        ...
        assert!(
            result.is_ok(),
            "wildcard re-export should allow any import name"
        );
    }
    ...
    fn namespace_import_skips_missing_check() {
        ...
        assert!(
            result.is_ok(),
            "namespace import should skip missing export check"
        );
    }
    ...
    fn link_result_contains_correct_export_names() {
        ...
        let exports = link
            .export_names
            .get(&dep_id)
            .expect("dep exports should exist");
