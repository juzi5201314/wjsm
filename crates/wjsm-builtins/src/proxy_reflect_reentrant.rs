//! Proxy trap 覆盖与 Reflect 同步再入算法（后端无关）。
//!
//! `Reflect.get/set/apply/construct/getPrototypeOf/setPrototypeOf/ownKeys`、
//! `Proxy.apply/construct` 的 Proxy 感知路径在此，由 native host 调度。
...
// ── Reflect.has (Proxy-aware) ─────────────────────────────────────────────

/// `Reflect.has(target, prop)` 的 Proxy 感知路径。
pub fn reflect_has<E: ExecContext>(ctx: &mut E, target: Value, prop: Value) -> Value {
    ...
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "has") {
            let result = match ctx.call_js(trap, handler, &[t, prop]) {
                Ok(v) => v,
        ...
        return reflect_has(ctx, t, prop);
    }
    // 非 proxy：委托同步实现
    crate::proxy_reflect::reflect_has_impl(ctx, target, prop)
}

// ── Reflect.get (Proxy-aware) ─────────────────────────────────────────────

/// `Reflect.get(target, prop, receiver)` 的 Proxy 感知路径。
///
/// 通过 `ExecContext::call_js` 同步再入；普通对象走 OrdinaryGet 算法，沿原型链
/// 查找属性槽并调用 accessor getter。
pub fn reflect_get_impl_with_receiver<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    prop: Value,
    receiver: Value,
    ...
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "get") {
            return match ctx.call_js(trap, handler, &[t, prop, receiver]) {
                Ok(v) => v,
                Err(_) => value::encode_undefined(),
            };
        }
        // 无 trap → 走 target 的 OrdinaryGet
        return ordinary_get(ctx, t, prop, receiver);
    }
    ordinary_get(ctx, target, prop, receiver)
}

/// §10.1.8 OrdinaryGet：沿原型链查找属性槽并同步调用 accessor getter。
fn ordinary_get<E: ExecContext>(ctx: &mut E, target: Value, prop: Value, receiver: Value) -> Value {
        ...
        || value::is_bound(target)
    {
            ...
            ctx.get_property_slot_on_proto(handle, name_id)
        {
            if is_accessor {
                ...
                return match ctx.call_js(getter, receiver, &[]) {
                    Ok(v) => v,
                    Err(_) => value::encode_undefined(),
                };
            }
...
// ── Reflect.set (Proxy-aware) ─────────────────────────────────────────────

/// `Reflect.set(target, prop, val, receiver)` 的 Proxy 感知路径。
pub fn reflect_set_impl_with_receiver<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    prop: Value,
    val: Value,
    ...
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "set") {
            let result = match ctx.call_js(trap, handler, &[t, prop, val, receiver]) {
                Ok(v) => v,
                Err(_) => return value::encode_bool(false),
            };
            return value::encode_bool(value::is_truthy(result));
        ...
        return value::encode_bool(ordinary_set_by_name_id(ctx, t, receiver, name_id, val));
    }
    let Some(name_id) = ctx.property_value_to_name_id(prop, true) else {
        return value::encode_bool(false);
    };
    value::encode_bool(ordinary_set_by_name_id(ctx, target, receiver, name_id, val))
}

// ── OrdinarySet / DefineValueOnReceiver ───────────────────────────────────

/// §10.1.9: OrdinarySet with OwnDescriptor 搜索（V2 堆路径）。
pub fn ordinary_set_by_name_id<E: ExecContext>(
    ctx: &mut E,
    obj: Value,
    receiver: Value,
    name_id: u32,
    ...
    loop {
        ...
        if let Some((_slot, flags, _getter, setter)) = ctx.get_own_property_slot(current, name_id) {
            if flags & wjsm_ir::constants::FLAG_IS_ACCESSOR as u32 != 0 {
                ...
                return ctx.call_js(setter, receiver, &[val]).is_ok();
            }
            ...
            return define_value_on_receiver(ctx, receiver, name_id, val);
        }
...
pub fn define_value_on_receiver<E: ExecContext>(
    ctx: &mut E,
    receiver: Value,
    name_id: u32,
    val: Value,
    ...
    if value::is_proxy(receiver) {
        ...
        let existing = reflect_get_own_property_descriptor_on_object(ctx, receiver, property);
...
/// `Reflect.apply(target, thisArg, args)` 的 Proxy 感知路径。
pub fn reflect_apply_impl<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    this_arg: Value,
    args: &[Value],
) -> Value {
    match ctx.call_js(target, this_arg, args) {
        Ok(v) => v,
        Err(_) => value::encode_undefined(),
    }
}

/// `Reflect.construct(target, args, newTarget)` 的 Proxy 感知路径。
pub fn reflect_construct_impl<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    args: &[Value],
    new_target: Value,
    ...
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "construct") {
            ...
            let result = match ctx.call_js(trap, handler, &[t, args_arr, new_target]) {
                Ok(v) => v,
        ...
        return ordinary_construct(ctx, t, args, new_target);
    }
    ordinary_construct(ctx, target, args, new_target)
}

/// 普通构造：分配 this_obj，设置原型，调用 target。
fn ordinary_construct<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    args: &[Value],
    new_target: Value,
    ...
    let proto_val = reflect_get_impl_with_receiver(ctx, new_target, proto_prop, new_target);
    ...
    let result = match ctx.call_js(target, this_obj, args) {
        Ok(v) => v,
...
/// `Reflect.getPrototypeOf(target)` 的 Proxy 感知路径。
pub fn reflect_get_prototype_of<E: ExecContext>(ctx: &mut E, target: Value) -> Value {
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "getPrototypeOf") {
            let res = match ctx.call_js(trap, handler, &[t]) {
                Ok(v) => v,
            ...
            if !ctx.is_extensible(t) {
                let target_proto = reflect_get_prototype_of_impl(ctx, t);
                if res != target_proto {
                    ctx.set_last_error(
        ...
        return reflect_get_prototype_of_impl(ctx, t);
    }
    reflect_get_prototype_of_impl(ctx, target)
}

/// 非 Proxy 的 getPrototypeOf 实现（递归 Proxy 感知）。
pub fn reflect_get_prototype_of_impl<E: ExecContext>(ctx: &mut E, target: Value) -> Value {
    if value::is_proxy(target) {
        return reflect_get_prototype_of(ctx, target);
    }
    // RegExp 原型
...
/// `Reflect.setPrototypeOf(target, proto)` 的 Proxy 感知路径。
pub fn reflect_set_prototype_of_fn_impl<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    proto: Value,
) -> Value {
    if value::is_proxy(target) {
        ...
        if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "setPrototypeOf") {
            let result = match ctx.call_js(trap, handler, &[t, proto]) {
                Ok(v) => v,
                Err(_) => return value::encode_bool(false),
            };
            let trap_res = value::is_truthy(result);
            if trap_res && !ctx.is_extensible(t) {
                let current_proto = reflect_get_prototype_of_impl(ctx, t);
...
pub fn proxy_own_keys_trap<E: ExecContext>(ctx: &mut E, target: Value) -> Value {
    if !value::is_proxy(target) {
        return value::encode_undefined();
    }
    ...
    let keys_val = match ctx.call_js(trap, handler, &[t]) {
        Ok(v) => v,
        Err(e) => {
            return ctx.make_type_error(&format!("Proxy ownKeys trap failed: {}", e));
        }
    };
    let keys = match extract_array_like_elements(ctx, keys_val) {
        Ok(arr) => arr,
...
/// 从数组或类数组对象提取元素列表。
pub fn extract_array_like_elements<E: ExecContext>(
    ctx: &mut E,
    arr_like: Value,
    ...
    if value::is_array(arr_like) {
        ...
        let len_val = reflect_get_impl_with_receiver(ctx, arr_like, len_prop, arr_like);
        ...
        for i in 0..len {
            ...
            let val = reflect_get_impl_with_receiver(ctx, arr_like, idx_prop, arr_like);
            elements.push(val);
        }
    }
    Ok(elements)
...
pub fn object_enumerable_own_keys<E: ExecContext>(ctx: &mut E, obj: Value) -> Value {
    if !value::is_js_object(obj) {
        return ctx.alloc_array(0);
    }
    if value::is_proxy(obj) {
        let keys_arr = proxy_own_keys_trap(ctx, obj);
        ...
        let keys = match extract_array_like_elements(ctx, keys_arr) {
            Ok(k) => k,
        ...
        for key in keys {
            ...
            let desc = reflect_get_own_property_descriptor_on_object(ctx, obj, key);
...
pub fn object_get_own_property_names<E: ExecContext>(ctx: &mut E, obj: Value) -> Value {
    if !value::is_js_object(obj) {
        return ctx.alloc_array(0);
    }
    if value::is_proxy(obj) {
...
pub fn object_values<E: ExecContext>(ctx: &mut E, obj: Value) -> Value {
    ...
    let keys_arr = object_enumerable_own_keys(ctx, obj);
    if value::is_exception(keys_arr) {
        return keys_arr;
    }
    let keys = match extract_array_like_elements(ctx, keys_arr) {
    ...
    for (i, key) in keys.iter().enumerate() {
        let val = reflect_get_impl_with_receiver(ctx, obj, *key, obj);
...
pub fn object_entries<E: ExecContext>(ctx: &mut E, obj: Value) -> Value {
    if !value::is_js_object(obj) {
        return ctx.alloc_array(0);
    }
    let keys_arr = object_enumerable_own_keys(ctx, obj);
...
pub fn object_assign_impl<E: ExecContext>(ctx: &mut E, target: Value, sources: &[Value]) -> Value {
    ...
    for &source_val in sources {
        ...
        for name in names {
            ...
            let prop_val = reflect_get_impl_with_receiver(ctx, source, name_val, source);
            let Some(name_id) = ctx.property_value_to_name_id(name_val, true) else {
                continue;
            };
            if !ordinary_set_by_name_id(ctx, target, target, name_id, prop_val) {
...
pub fn reflect_get_own_property_descriptor_on_object<E: ExecContext>(
    ctx: &mut E,
    target: Value,
    prop: Value,
) -> Value {
    if value::is_proxy(target) {
        ...
        if !value::is_undefined(trap) && !value::is_null(trap) {
            let descriptor = match ctx.call_js(trap, handler, &[t, prop]) {
                Ok(d) => d,
                Err(e) => {
...
// ── Reentrant Proxy traps（$obj_get / $obj_set / $obj_delete）─────────────

/// `proxy_trap_get`：Proxy [[Get]] 内部方法。
pub fn proxy_trap_internal_get<E: ExecContext>(ctx: &mut E, proxy: Value, name_id: i32) -> Value {
    ...
    if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "get") {
        ...
        return match ctx.call_js(trap, handler, &[target, prop, proxy]) {
            Ok(v) => v,
            Err(_) => value::encode_undefined(),
        };
    }
    // 无 trap → target OrdinaryGet 路径。
    let prop = proxy_trap_property_key_value(ctx, name_id);
    ordinary_get(ctx, target, prop, proxy)
}

/// `proxy_trap_set`：Proxy [[Set]] 内部方法（返回 void，不可捕获异常）。
pub fn proxy_trap_internal_set<E: ExecContext>(
    ctx: &mut E,
    proxy: Value,
    name_id: i32,
    val: Value,
    ...
    if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "set") {
        let prop = proxy_trap_property_key_value(ctx, name_id);
        let _ = ctx.call_js(trap, handler, &[target, prop, val, proxy]);
        return;
    }
    ...
    let _ = ordinary_set_by_name_id(ctx, target, proxy, nid, val);
}

/// `proxy_trap_delete`：Proxy [[Delete]] 内部方法。
pub fn proxy_trap_internal_delete<E: ExecContext>(
    ctx: &mut E,
    proxy: Value,
    name_id: i32,
) -> Value {
    ...
    if let Some(trap) = proxy_trap_handler_trap(ctx, handler, "deleteProperty") {
        let prop = proxy_trap_property_key_value(ctx, name_id);
        let result = match ctx.call_js(trap, handler, &[target, prop]) {
            Ok(v) => v,
            Err(_) => return value::encode_bool(false),
        };
        return value::encode_bool(value::is_truthy(result));
    }
    crate::property::delete_by_name_id(ctx, target, name_id as u32)
}
