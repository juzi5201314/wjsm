//! `in` 与 iterator 的同步再入 builtins。
//!
//! Proxy `has` trap 与 ObjectIter `.next`/`.return` 经 `call_js` 再入；
...
pub fn op_in<E: ExecContext>(ctx: &mut E, object: Value, prop: Value) -> Value {
    if value::is_proxy(object) {
        ...
        if !value::is_undefined(trap) && !value::is_null(trap) {
            let result = ctx
                .call_js(trap, entry.handler, &[entry.target, prop])
                .unwrap_or_else(|_| value::encode_bool(false));
            return value::encode_bool(ctx.to_boolean(result));
        }
        return op_in(ctx, entry.target, prop);
    }
    crate::core::ordinary_has_property(ctx, object, prop)
}

pub fn iterator_from<E: ExecContext>(ctx: &mut E, val: Value) -> Value {
    ...
    iterator_from_fallback(ctx, val)
}

fn iterator_from_fallback<E: ExecContext>(ctx: &mut E, val: Value) -> Value {
    ...
    let iterator = match ctx.call_js(method, val, &[]) {
...
pub fn materialize_async_from_sync_next<E: ExecContext>(ctx: &mut E, afs: u32) -> Value {
    let outer = ctx
        .async_from_sync_outer_iterator(afs)
        .unwrap_or_else(|| value::encode_handle(value::TAG_ITERATOR, afs));
    let promise = ctx.advance_async_from_sync(afs);
...
pub fn iterator_next<E: ExecContext>(ctx: &mut E, handle: Value) -> Value {
    ...
    match ctx.iterator_next_sync_step(handle) {
        ...
        IteratorNextStep::NeedAsyncFromSync { afs } => materialize_async_from_sync_next(ctx, afs),
        IteratorNextStep::NeedObjectNext { iterator, next } => {
            advance_object_iterator_next(ctx, handle, iterator, next)
        }
    }
}

fn advance_object_iterator_next<E: ExecContext>(
    ctx: &mut E,
    handle: Value,
    iterator: Value,
    next: Value,
...
pub fn iterator_done<E: ExecContext>(ctx: &mut E, handle: Value) -> Value {
...
pub fn iterator_close<E: ExecContext>(ctx: &mut E, handle: Value, completion: Value) -> Value {
    ...
    let result = match ctx.call_js(return_method, iterator, &[]) {
        Ok(v) => v,
        Err(_) => value::encode_undefined(),
    };
...
pub fn iterator_step_value<E: ExecContext>(ctx: &mut E, handle: Value) -> Value {
    let done = iterator_done(ctx, handle);
    if value::decode_bool(done) {
        return value::encode_undefined();
    }
    let value = ctx.iterator_current_value(handle);
    let _ = iterator_next(ctx, handle);
    value
}
