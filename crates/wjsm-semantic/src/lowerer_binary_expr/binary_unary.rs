        ...
        match expr {
            ...
            swc_ast::Expr::Unary(unary) => match unary.op {
                ...
                swc_ast::UnaryOp::Delete => {
                    // 括号/TS 包装透传 Reference：`delete (o.x)` 与 `delete o.x`
                    // 同为 DeleteProp（strict 不可配置属性可抛）；可选链 delete
                    // 的异常已在链内分叉，标识符/非引用目标的异常均在降级点
                    // 就地分叉，结果不携带哨兵。
                    matches!(
                        super::delete_expr::delete_reference_target(unary.arg.as_ref()),
                        swc_ast::Expr::Member(_)
                    )
                }
                _ => false,
...
impl Lowerer {
    ...
    pub(crate) fn lower_unary(
        ...
        match unary.op {
            ...
            // delete 操作符：目标分类与 DeleteBinding 裁决见 delete_expr.rs。
            Delete => self.lower_delete(unary, block),
        }
    }
