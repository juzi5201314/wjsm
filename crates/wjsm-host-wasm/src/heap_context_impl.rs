//! `Caller<RuntimeState>` 上的 [`wjsm_host::HeapContext`] 适配器。
//!
//! 将后端无关的堆/侧表操作委托到现有 host-wasm 实现（heap_access_v2、
//! runtime_heap、async_hooks、host temp roots 等）。

use crate::runtime_string::RuntimeString;
use crate::{RuntimeState, WasmEnv, value};
use wasmtime::Caller;
use wjsm_host::{AsyncHookEvent, GcOutcome, Handle, HeapContext, Value};

/// Wasmtime 后端的 [`HeapContext`] 实现。
///
/// `env` 在构造时从 Caller 提取（`WasmEnv: Copy`），避免后续方法因
/// `get_export` 需要 `&mut Caller` 而无法读取线性内存。
///
/// `HeapContext` 方法均为 `&mut self`，因此可直接持有 `&mut Caller`，
/// 无需 raw 指针或内部可变性。
#[allow(dead_code)]
pub(crate) struct WasmHeapContext<'a, 'b> {
    caller: &'a mut Caller<'b, RuntimeState>,
    env: Option<WasmEnv>,
}

#[allow(dead_code)]
impl<'a, 'b> WasmHeapContext<'a, 'b> {
...
impl HeapContext for WasmHeapContext<'_, '_> {
    ...
    fn gc_collect(&mut self) -> GcOutcome {
        ...
        GcOutcome {
            // 本轮为 1；累计轮数由 RuntimeState 观测序列维护，此处给最小可用投影。
            cycle_count: 1,
    ...
    fn async_hook_callbacks(&mut self, event: AsyncHookEvent, promise: bool) -> Vec<Value> {
        // 与 runtime_async_hooks/emit.rs::callback_for 同规则的简化投影：
        // 过滤 enabled / track_promises，按事件取非零回调。
        let hooks = self
            .caller
            .data()
            .async_hooks
