...
pub(crate) fn define_async_fn(
         ...
         (fn_table_idx, continuation, state, resume_val, completion_raw): (i64, i64, i64, i64, i64)| {
            Box::new(async move {
                ...
                wjsm_builtins::async_fn::async_function_resume(
                    &mut ctx, fn_table_idx, continuation, state, resume_val, completion_raw,
                )
                .await
            })
        },
    ...
    let async_function_suspend_fn = Func::wrap(
        &mut store,
        |mut caller: Caller<'_, RuntimeState>, continuation: i64, awaited_promise: i64, state: i64| {
            let mut ctx = WasmExecContext::new(&mut caller);
            wjsm_builtins::async_fn::async_function_suspend(
                &mut ctx, continuation, awaited_promise, state,
            )
        },
    );
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            wjsm_builtins::async_fn::continuation_create(
                &mut ctx, fn_table_idx, outer_promise, captured_var_count,
            )
        },
    );
