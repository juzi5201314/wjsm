...
// `MemoryFootprintSample` 与 `GcExecutionStats` 的单一来源是 wjsm-gc（api.rs）；
// 此处 re-export 保持 host-wasm 对外 API 不变。
pub use wjsm_gc::api::{GcExecutionStats, MemoryFootprintSample};
