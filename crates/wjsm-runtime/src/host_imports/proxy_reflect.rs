...
pub(crate) fn define_proxy_reflect(
         ...
         -> i64 {
            if value::is_proxy(target) {
                ...
                if let Some(entry) = entry {
                    ...
                    if let Some(handler_ptr) = resolve_handle(&mut caller, entry.handler) {
                        let trap = read_object_property_by_name(&mut caller, handler_ptr, "set")
                            .unwrap_or_else(value::encode_undefined);
                        if !value::is_undefined(trap) && !value::is_null(trap) {
    ...
    linker.define(
        ...
        reflect_delete_property_fn,
    )?;
    fn reflect_delete_property_impl(
        caller: &mut Caller<'_, RuntimeState>,
        target: i64,
        prop: i64,
    ) -> i64 {
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            if !is_callable_in_runtime(&mut caller, target) {
                set_runtime_error(
                    caller.data(),
                    "TypeError: Reflect.apply target must be callable".to_string(),
                );
                return value::encode_undefined();
            }
                ...
                if let Some(entry) = entry {
                    ...
                    if let Some(handler_ptr) = resolve_handle(&mut caller, entry.handler) {
                        ...
                        if !value::is_undefined(trap) && !value::is_null(trap) {
                            ...
                            let trap_result = call_wasm_callback(
                                &mut caller,
                                trap,
                                entry.handler,
    ...
    let reflect_construct_fn = Func::wrap(
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            if !is_constructor_in_runtime(&mut caller, target)
                || !is_constructor_in_runtime(&mut caller, n_target)
            {
                set_runtime_error(
                    caller.data(),
    ...
    fn reflect_construct_impl(
        ...
        let shadow_sp_global = caller
            .get_export("__shadow_sp")
            .and_then(|e| e.into_global())
            .expect("__shadow_sp in reflect_construct_impl");
        let saved_sp = shadow_sp_global
            .get(&mut *caller)
            .i32()
            .expect("shadow_sp i32 in reflect_construct_impl");
        let memory = caller
            .get_export("memory")
            .and_then(|e| e.into_memory())
            .expect("memory in reflect_construct_impl");
        for (i, &arg) in args.iter().enumerate() {
            memory
                .write(
                    &mut *caller,
                ...
                .expect("shadow stack write in reflect_construct_impl");
        }
        shadow_sp_global
            .set(&mut *caller, Val::I32(saved_sp + args.len() as i32 * 8))
            .expect("shadow_sp set in reflect_construct_impl");
        let result = resolve_and_call(caller, target, this_obj, saved_sp, args.len() as i32);
        shadow_sp_global
            .set(&mut *caller, Val::I32(saved_sp))
            .expect("shadow_sp restore in reflect_construct_impl");
                        ...
                        if !value::is_undefined(trap) && !value::is_null(trap) {
                            ...
                            let trap_res = match result {
                                ...
                                Err(e) => {
                                    set_runtime_error(
                                        caller.data(),
                                        format!("TypeError: setPrototypeOf trap failed: {}", e),
                                    );
                                    return value::encode_bool(false);
                                }
                            };
                            if trap_res {
                                ...
                                if !ext {
                                    let current_proto =
                                        reflect_get_prototype_of_impl(&mut caller, entry.target);
    ...
    let reflect_is_extensible_fn = Func::wrap(
        ...
        |mut caller: Caller<'_, RuntimeState>, target: i64| -> i64 {
            ...
            value::encode_bool(proxy_or_target_is_extensible_impl(&mut caller, target))
        },
    );
    linker.define(
        &mut store,
    ...
    let reflect_prevent_extensions_fn = Func::wrap(
        ...
        |mut caller: Caller<'_, RuntimeState>, target: i64| -> i64 {
            ...
            value::encode_bool(proxy_or_target_prevent_extensions_impl(&mut caller, target))
        },
    );
    linker.define(
        &mut store,
        ...
        |mut caller: Caller<'_, RuntimeState>, target: i64, prop: i64| -> i64 {
            ...
            if value::is_proxy(target) {
                ...
                if let Some(entry) = entry {
                    ...
                    if let Some(handler_ptr) = resolve_handle(&mut caller, entry.handler) {
                        ...
                        if !value::is_undefined(trap) && !value::is_null(trap) {
                            ...
                            let descriptor = match trap_result {
                                ...
                                Err(e) => {
                                    set_runtime_error(
                                        caller.data(),
                                        format!(
                                            "TypeError: getOwnPropertyDescriptor trap failed: {}",
                                            e
                                        ),
                                    );
                                    return value::encode_undefined();
                            ...
                            let name_id = prop_name
                                .as_deref()
                                .and_then(|name| find_memory_c_string(&mut caller, name));
                    ...
                    return reflect_get_own_property_descriptor_impl(
                        &mut caller,
                        entry.target,
                        prop,
                    );
    ...
    linker.define(
        ...
        reflect_get_own_property_descriptor_fn,
    )?;
    fn reflect_get_own_property_descriptor_impl(
        caller: &mut Caller<'_, RuntimeState>,
        target: i64,
        prop: i64,
    ) -> i64 {
        ...
        |mut caller: Caller<'_, RuntimeState>, target: i64| -> i64 {
            if value::is_proxy(target) {
                ...
                if let Some(entry) = entry {
                    ...
                    if let Some(handler_ptr) = resolve_handle(&mut caller, entry.handler) {
                        let trap =
                            read_object_property_by_name(&mut caller, handler_ptr, "ownKeys")
                                .unwrap_or_else(value::encode_undefined);
                        if !value::is_undefined(trap) && !value::is_null(trap) {
                            ...
                            if !ext {
                                ...
                                for tk in &target_keys {
                                    if let Some(tk_c) = find_memory_c_string(&mut caller, tk) {
                                        if let Some((_, flags, _)) =
                                            find_property_slot_by_name_id(&mut caller, t_ptr, tk_c)
                                        {
                                            let configurable =
                                                (flags & constants::FLAG_CONFIGURABLE) != 0;
    ...
    let proxy_construct_fn = Func::wrap(
        &mut store,
        |mut caller: Caller<'_, RuntimeState>,
         proxy: i64,
         _this_val: i64,
