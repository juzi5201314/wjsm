        ...
        ($name:ident, $import:literal, $size:expr, $kind:expr) => {
            let $name = Func::wrap(
                ...
                |mut caller: Caller<'_, RuntimeState>,
                 ...
                 length: i64|
                 -> i64 {
                    let elem_size = $size as u32;
                    let mut initial_values: Option<Vec<i64>> = None;
                    let (buf_handle, offset, len) = if value::is_array(buffer) {
                        let Some(arr_ptr) = resolve_array_ptr(&mut caller, buffer) else {
                            return value::encode_undefined();
                        };
                        let len = read_array_length(&mut caller, arr_ptr).unwrap_or(0);
                        ...
                        let handle = {
                            ...
                            table.push(ArrayBufferEntry {
                                data: vec![0; len.saturating_mul(elem_size) as usize],
                            });
                            handle
                        };
                        initial_values = Some(values);
                        (handle, 0, len)
                    } else if let Some(src_entry) = typedarray_entry_from_value(&mut caller, buffer)
                    {
                        let len = src_entry.length;
                        let mut values = Vec::with_capacity(len as usize);
                        for i in 0..len {
                            values.push(
                                typedarray_element_read(&mut caller, buffer, i)
                                    .unwrap_or_else(value::encode_undefined),
                            );
                        }
                        let handle = {
                        ...
                        let offset_raw = if value::is_undefined(byte_offset) {
                            0.0
                        } else {
                            value::decode_f64(byte_offset)
                        };
                        if offset_raw < 0.0 {
                            *caller.data().runtime_error.lock().expect("error mutex") =
                                Some("RangeError: Invalid typed array byteOffset".to_string());
                            return value::encode_undefined();
                        }
                        let offset = offset_raw as u32;
                        let obj_ptr = resolve_handle(&mut caller, buffer);
                        match obj_ptr {
                            Some(ptr) => {
                                let h = read_object_property_by_name(
                                ...
                                let bl =
                                    read_object_property_by_name(&mut caller, ptr, "byteLength");
                                match (h, bl) {
                                    (Some(handle_val), Some(byte_len_val)) => {
                                        let byte_len = value::decode_f64(byte_len_val) as u32;
                                        if offset > byte_len || offset % elem_size != 0 {
                                            *caller
                                                .data()
                                                .runtime_error
                                                .lock()
                                                .expect("error mutex") = Some(
                                        ...
                                        let len = if value::is_undefined(length) {
                                            ...
                                            let len_raw = value::decode_f64(length);
                                            if len_raw < 0.0 {
                                                *caller
                                                    .data()
                                                    .runtime_error
                                            ...
                                            let len = len_raw as u32;
                                            let Some(byte_count) = len.checked_mul(elem_size)
                                            else {
                                                *caller
                                                    .data()
                                                    .runtime_error
                                        ...
                                        (value::decode_f64(handle_val) as u32, offset, len)
                                    }
                                    _ => return value::encode_undefined(),
                                }
                            }
                        ...
                        let len = if value::is_undefined(buffer) {
                            0
                        } else {
                            value::decode_f64(buffer) as u32
                        };
                        let handle = {
                            let mut table = caller
                                .data()
                    ...
                    let _ =
                        define_host_data_property_from_caller(&mut caller, obj, "length", len_val);
                    let bl_val = value::encode_f64((len * elem_size) as f64);
                    let _ = define_host_data_property_from_caller(
                        &mut caller,
                        obj,
                        "byteLength",
                    ...
                    if let Some(values) = initial_values {
                        for (i, value) in values.into_iter().enumerate() {
                            let _ = typedarray_element_write(&mut caller, obj, i as u32, value);
                        }
                    }
                    obj
                },
