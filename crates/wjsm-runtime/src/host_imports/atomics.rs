...
pub(crate) fn define_atomics(
        ...
        |mut caller: Caller<'_, RuntimeState>, this_val: i64, index_val: i64| -> i64 {
            ...
            let val: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .load(Ordering::SeqCst) as f64,
                    (1, 1) | (1, 2) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .load(Ordering::SeqCst) as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .load(Ordering::SeqCst) as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .load(Ordering::SeqCst) as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .load(Ordering::SeqCst) as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .load(Ordering::SeqCst) as f64,
                    (4, 3) => f32::from_bits(
                        (&*(addr as *const std::sync::atomic::AtomicU32)).load(Ordering::SeqCst),
                    ) as f64,
                    (8, 3) => f64::from_bits(
                        (&*(addr as *const std::sync::atomic::AtomicU64)).load(Ordering::SeqCst),
                    ),
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            unsafe {
                let addr = ptr.add(off);
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *mut std::sync::atomic::AtomicI8))
                        .store(f_raw as i8, Ordering::SeqCst),
                    (1, 1) | (1, 2) => (&*(addr as *mut std::sync::atomic::AtomicU8))
                        .store(f_raw as u8, Ordering::SeqCst),
                    (2, 0) => (&*(addr as *mut std::sync::atomic::AtomicI16))
                        .store(f_raw as i16, Ordering::SeqCst),
                    (2, 1) => (&*(addr as *mut std::sync::atomic::AtomicU16))
                        .store(f_raw as u16, Ordering::SeqCst),
                    (4, 0) => (&*(addr as *mut std::sync::atomic::AtomicI32))
                        .store(f_raw as i32, Ordering::SeqCst),
                    (4, 1) => (&*(addr as *mut std::sync::atomic::AtomicU32))
                        .store(f_raw as u32, Ordering::SeqCst),
                    (4, 3) => (&*(addr as *mut std::sync::atomic::AtomicU32))
                        .store((f_raw as f32).to_bits(), Ordering::SeqCst),
                    (8, 3) => (&*(addr as *mut std::sync::atomic::AtomicU64))
                        .store(f_raw.to_bits(), Ordering::SeqCst),
                    _ => {}
                }
            }
            value_val
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .fetch_add(f_raw as i8, Ordering::SeqCst)
                        as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .fetch_add(f_raw as u8, Ordering::SeqCst)
                        as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .fetch_add(f_raw as i16, Ordering::SeqCst)
                        as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .fetch_add(f_raw as u16, Ordering::SeqCst)
                        as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .fetch_add(f_raw as i32, Ordering::SeqCst)
                        as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .fetch_add(f_raw as u32, Ordering::SeqCst)
                        as f64,
                    _ => return value::encode_undefined(),
                }
            };
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .fetch_sub(f_raw as i8, Ordering::SeqCst)
                        as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .fetch_sub(f_raw as u8, Ordering::SeqCst)
                        as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .fetch_sub(f_raw as i16, Ordering::SeqCst)
                        as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .fetch_sub(f_raw as u16, Ordering::SeqCst)
                        as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .fetch_sub(f_raw as i32, Ordering::SeqCst)
                        as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .fetch_sub(f_raw as u32, Ordering::SeqCst)
                        as f64,
                    _ => return value::encode_undefined(),
                }
            };
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .fetch_and(f_raw as i8, Ordering::SeqCst)
                        as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .fetch_and(f_raw as u8, Ordering::SeqCst)
                        as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .fetch_and(f_raw as i16, Ordering::SeqCst)
                        as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .fetch_and(f_raw as u16, Ordering::SeqCst)
                        as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .fetch_and(f_raw as i32, Ordering::SeqCst)
                        as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .fetch_and(f_raw as u32, Ordering::SeqCst)
                        as f64,
                    _ => return value::encode_undefined(),
                }
            };
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .fetch_or(f_raw as i8, Ordering::SeqCst)
                        as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .fetch_or(f_raw as u8, Ordering::SeqCst)
                        as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .fetch_or(f_raw as i16, Ordering::SeqCst)
                        as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .fetch_or(f_raw as u16, Ordering::SeqCst)
                        as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .fetch_or(f_raw as i32, Ordering::SeqCst)
                        as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .fetch_or(f_raw as u32, Ordering::SeqCst)
                        as f64,
                    _ => return value::encode_undefined(),
                }
            };
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .fetch_xor(f_raw as i8, Ordering::SeqCst)
                        as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .fetch_xor(f_raw as u8, Ordering::SeqCst)
                        as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .fetch_xor(f_raw as i16, Ordering::SeqCst)
                        as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .fetch_xor(f_raw as u16, Ordering::SeqCst)
                        as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .fetch_xor(f_raw as i32, Ordering::SeqCst)
                        as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .fetch_xor(f_raw as u32, Ordering::SeqCst)
                        as f64,
                    _ => return value::encode_undefined(),
                }
            };
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let old: f64 = unsafe {
                ...
                match (elem_size, element_kind) {
                    (1, 0) => (&*(addr as *const std::sync::atomic::AtomicI8))
                        .swap(f_raw as i8, Ordering::SeqCst) as f64,
                    (1, 1) => (&*(addr as *const std::sync::atomic::AtomicU8))
                        .swap(f_raw as u8, Ordering::SeqCst) as f64,
                    (2, 0) => (&*(addr as *const std::sync::atomic::AtomicI16))
                        .swap(f_raw as i16, Ordering::SeqCst) as f64,
                    (2, 1) => (&*(addr as *const std::sync::atomic::AtomicU16))
                        .swap(f_raw as u16, Ordering::SeqCst) as f64,
                    (4, 0) => (&*(addr as *const std::sync::atomic::AtomicI32))
                        .swap(f_raw as i32, Ordering::SeqCst) as f64,
                    (4, 1) => (&*(addr as *const std::sync::atomic::AtomicU32))
                        .swap(f_raw as u32, Ordering::SeqCst) as f64,
                    _ => return value::encode_undefined(),
                }
            };
            value::encode_f64(old)
    ...
    let atomics_wait_async_fn = Func::wrap(
        ...
        |mut caller: Caller<'_, RuntimeState>,
         ...
         -> i64 {
            ...
            let _ = define_host_data_property_from_caller(&mut caller, result, "value", status_str);
            result
        },
    );
    linker.define(
    ...
    let sab_byte_length_stub = Func::wrap(
        ...
        |mut caller: Caller<'_, RuntimeState>, this_val: i64| -> i64 {
            ...
            let obj_ptr = match resolve_handle_idx(
                &mut caller,
