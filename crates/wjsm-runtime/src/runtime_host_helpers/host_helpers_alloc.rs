...
pub(crate) async fn array_species_create_async(
    ...
    let len_val = value::encode_f64(length as f64);
    let Some(shadow_sp) = push_args_to_shadow_stack(caller, &env, &[len_val]) else {
        return value::encode_undefined();
    };
    ...
    restore_shadow_sp(caller, &env, shadow_sp);
    caller
        .data()
        .new_target
...
pub(crate) fn alloc_array_with_env<C: AsContextMut<Data = RuntimeState>>(
    ...
    #[cfg(feature = "managed-heap-v2")]
    {
        ...
        let Ok((object, _)) =
            crate::runtime_heap::allocate_v2_object_bytes_with_context(ctx, bytes)
        else {
            return value::encode_undefined();
        };
        if env
            ...
            .is_err()
            || access
                .publish_array(handle as u32, object, proto as u32, capacity)
                .is_err()
        {
            return value::encode_undefined();
        }
...
pub(crate) fn set_array_elem_with_env<C: AsContextMut<Data = RuntimeState>>(
    ...
    if value::is_array(arr_val) {
        ...
        if ctx
            ...
            .data()
            .heap_access_v2()
            .set_element(handle, index as u32, val as u64)
            .is_ok()
        {
            return;
        }
...
pub(crate) fn alloc_object_with_env<C: AsContextMut<Data = RuntimeState>>(
    ...
    #[cfg(feature = "managed-heap-v2")]
    {
        ...
        if env
            ...
            .is_err()
            || access
                .publish_object(handle as u32, object, proto as u32, capacity)
                .is_err()
        {
            return value::encode_undefined();
        }
