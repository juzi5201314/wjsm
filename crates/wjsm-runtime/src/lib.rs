...
fn read_i32_global_from_caller(caller: &mut Caller<'_, RuntimeState>, name: &str) -> Option<i32> {
    caller
        .get_export(name)
...
fn read_utf8_slice(data: &[u8], ptr: u32, len: u32) -> Option<String> {
    let start = ptr as usize;
...
fn read_eval_var_map(caller: &mut Caller<'_, RuntimeState>) -> Vec<EvalVarMapEntry> {
...
fn try_compiled_eval_from_caller(
    caller: &mut Caller<'_, RuntimeState>,
    code: &str,
...
fn reserve_eval_data_segment(caller: &mut Caller<'_, RuntimeState>, code_len: u32) -> Result<u32> {
    let heap_ptr = caller
        .get_export("__heap_ptr")
        .and_then(Extern::into_global)
...
fn format_eval_error(error: anyhow::Error) -> String {
...
fn runtime_module_has_use_strict_directive(module: &swc_ast::Module) -> bool {
...
fn eval_lit(caller: &Caller<'_, RuntimeState>, lit: &swc_ast::Lit) -> Result<i64, String> {
    match lit {
...
fn eval_unary(op: swc_ast::UnaryOp, val: i64) -> Result<i64, String> {
    match op {
...
fn pat_ident_name(pat: &swc_ast::Pat) -> Option<&str> {
    match pat {
...
fn eval_scope_has_strict_marker(caller: &mut Caller<'_, RuntimeState>, scope_env: i64) -> bool {
...
fn create_eval_function(state: &RuntimeState, function: EvalFunction) -> i64 {
    let mut table = state
        .native_callables
        .lock()
        .expect("native callable table mutex");
...
fn call_eval_function_from_caller(
    caller: &mut Caller<'_, RuntimeState>,
...
fn set_host_data_property_from_caller(
    caller: &mut Caller<'_, RuntimeState>,
    obj: i64,
    name: &str,
    val: i64,
...
fn eval_to_string(caller: &mut Caller<'_, RuntimeState>, val: i64) -> String {
    if value::is_string(val) {
...
fn call_host_function_from_caller(
    caller: &mut Caller<'_, RuntimeState>,
    func_table: &Table,
    handler: i64,
    argument: i64,
