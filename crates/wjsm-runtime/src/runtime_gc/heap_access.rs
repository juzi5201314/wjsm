...
fn handle_for_ptr<C: AsContextMut<Data = RuntimeState>>(
    ...
    for h in 0..obj_table_count {
        let slot = obj_table_ptr.checked_add(h.checked_mul(constants::HANDLE_TABLE_ENTRY_SIZE as usize)?)?;
...
fn read_obj_table_ptr(ctx: &mut GcContext<'_>, h: Handle) -> Option<usize> {
    ...
    let slot_addr = obj_table_ptr.checked_add(h as usize * constants::HANDLE_TABLE_ENTRY_SIZE as usize)?;
...
fn write_u32(ctx: &mut GcContext<'_>, addr: usize, val: u32) -> Option<()> {
    ctx.with_memory_mut(|data| {
        data.get_mut(addr..addr + 4)?.copy_from_slice(&val.to_le_bytes());
        Some(())
    })
}
...
fn write_i64(ctx: &mut GcContext<'_>, addr: usize, val: Value) -> Option<()> {
    ctx.with_memory_mut(|data| {
        data.get_mut(addr..addr + 8)?.copy_from_slice(&val.to_le_bytes());
        Some(())
    })
}
