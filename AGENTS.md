...
**Quick JS snippets (agents)**: For ad-hoc **execution**, use inline source—**never** `echo '…' > /tmp/foo.js && cargo run -- run /tmp/foo.js`. Only `run` and `eval` accept inline code: `cargo run -- run -e '<statements>'` (scripts; add `--script` when `await` must be an identifier) or `cargo run -- eval '<expression>'` (single expression, wrapped as `console.log((expr))`). Both use the eval compile pipeline (`lower_eval_module` / `compile_eval`), not multi-file bundling. **`dump-ir` / `dump-wat` / `dump-ast` / `check` / `build` have no `-e`**—use a repo path (`fixtures/…`) or stdin: `printf '%s' '<source>' | cargo run -- dump-ir -` (same module lowering as `run <file>`, not the eval pipeline). `/tmp/*.wasm` and other build artifacts in `/tmp` are fine; do not put scratch `.js` in the project tree.
