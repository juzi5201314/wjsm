...
### Covered by fixtures
**Happy path** (308 fixtures): 287 `.js` + 21 `.ts`/`.tsx` (TS types, JSX, `using`, Proxy variants). Covers functions, closures (lexical capture, arrow this), classes (ctor, methods, super; getter/setter and static blocks have only basic coverage), async/await, async generators, top-level await, Promises (all combinators + microtask ordering + withResolvers), modules (ESM + CJS + dynamic import), objects (property descriptors, prototype chains, computed props, spread), arrays (literal + all ES Array.prototype methods), template strings (tagged), destructuring (array, object, params, defaults), control flow (if/else, switch, while/do-while/for/for-in/for-of, break/continue/labeled, try/catch/finally/throw), eval (direct/indirect/strict, many variants), BigInt, Symbol, RegExp (flags, lookbehind, named groups, unicode properties), TypedArray (all numeric types + BigInt variants), Date, Map/Set (with groupBy), Math, Reflect API, Proxy (full traps + invariants + revocable), WeakRef, FinalizationRegistry, SharedArrayBuffer, Atomics, Object/Array/String built-in methods, typeof, operators, TypeScript (type annotations, enum, interface, namespace, type alias, type assertions), JSX, `using` declarations.
**Note on gaps** (implemented in runtime but thin/no dedicated E2E fixtures): timers (`setTimeout`/`setInterval`/`clear*` — host support exists but no happy fixtures), deep class static/getter/setter/static-block combinations, some Array.prototype edge methods, extended Date methods, extended console methods.
