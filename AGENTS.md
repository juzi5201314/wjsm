...
**Note on gaps** (normative gaps or implementation limitations — documented in fixtures):
  - fetch: data: URLs plus HTTP/HTTPS streaming. Response.body is a canonical ReadableStream byte stream; reader.read(), text(), json(), and arrayBuffer() all consume the HTTP body with backpressure via AsyncOpGuard. Default and BYOB readers supported. **Limitation**: BYOB reader.read(view) result view length currently equals the supplied view capacity rather than spec-correct bytes-written; overflow bytes are preserved for subsequent reads but the returned view is not truncated to bytesWritten.
  - ReadableStream BYOB: `controller.byobRequest` is a getter returning the active `ReadableStreamBYOBRequest` (with `.view` and `.respond(bytesWritten)`). Underlying source `pull(controller)` is invoked asynchronously via `ReadableStreamPull` microtask when a BYOB read is pending. **Limitation**: result view length (see fetch above).
