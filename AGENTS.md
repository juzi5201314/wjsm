...
**GC / ManagedHeap** ([ADR 0010](docs/adr/0010-generational-zgc-managed-heap.md); supersedes ADR 0005): product path targets a unified ManagedHeap on Wasmtime shared memory64 with 8-byte atomic handles, fixed GC worker pool, and Generational ZGC (young/old mark, remset, concurrent relocate, pacing director). Public selection remains `RuntimeOptions` / CLI `--gc` / `WJSM_GC` (`mark-sweep`, `g1`, `zgc`); `WJSM_TEST_GC` is tests-only. Performance evidence is **only** `wjsm-gc-bench` (`preflight`/`compare`/`gate`); legacy `gc_stress` / `zgc_autoresearch` / `zgc_barrier_pressure` are retired. PR heaps 32/256/1024 MiB; 4/16 GiB nightly needs hard isolation runners (exit 78 = `needs-resource-runner`, never shrink matrix or auto-pass). Missing JDK diagnostic counters or named capability runners stay `needs-verification` / `needs-capability-runner`. Active V2 cutover (delete private `managed-heap-v2` + memory32 main path) is Task 15 — do not reintroduce dual-heap fallbacks. **Feature hygiene**: no workspace crate may unconditionally enable `managed-heap-v2` in a dependency declaration — cargo feature unification would silently switch every `--workspace` build/test onto the in-development V2 heap (this caused a 100+ test failure incident; see `docs/aegis/work/2026-07-16-zgc-performance/32-root-cause-feature-unification.md`). V2 consumers must forward it via their own opt-in feature plus `required-features` (bin/test targets) or module-level `#[cfg]` gating (libs); V2 builds/tests/benches always pass `--features managed-heap-v2` explicitly (e.g. `cargo run -p wjsm-gc-bench --features managed-heap-v2`).
