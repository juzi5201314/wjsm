...
> <details><summary>为什么不统一一条「CLI > env > file > default」优先级链？</summary>
>
> 看起来统一一条链更直观，实际工程里行不通。原因是：
>
> - `--gc` 和 `WJSM_GC` 是同一件事，命令行能覆盖环境变量。
> - 但 `--max-heap-size` 和 `WJSM_MAX_HEAP_SIZE` 根本不是一对——wjsm 没有 `WJSM_MAX_HEAP_SIZE` 这个变量。要给「同一个设置」写统一的覆盖规则，前提是「存在同一个设置的多种来源」。
>
> 实际看下来，wjsm 的「命令行 vs 文件」是一对、「命令行 vs 环境变量」是另一对，两对之间没有交叉项。强行合并成一条链要么编造不存在的来源对，要么写一堆「此选项不适用于该来源」的特例。
>
> 把它们分开说（配置文件只跟命令行比、环境变量只跟命令行比）虽然文字描述不优雅，但和实现完全一致。
>
> </details>
