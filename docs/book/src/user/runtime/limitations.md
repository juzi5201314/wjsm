...
Buffer 家族的长度 / 偏移参数完整执行 ToIndex（§7.1.22）：`ArrayBuffer` / `SharedArrayBuffer` / `DataView` 构造器与 `resize` / `transfer` / `grow` / DataView `get*` / `set*` / `slice` 都会跑 ToIntegerOrInfinity 的用户转换（`@@toPrimitive` / `valueOf` / `toString`，`options.maxByteLength` 经 Get 触发 getter），Symbol / BigInt 按 V8 文案抛 `TypeError`，越界（负值 / ≥2^53）按各站点 V8 文案抛 `RangeError`；检查顺序（转换副作用先行、detach `TypeError` 与边界 `RangeError` 的先后）对齐 Node v22 实测。合法但不可分配的长度按 V8 文案 `Array buffer allocation failed` 抛可恢复 `RangeError`，绝不触发宿主 allocator abort。已知差异：宿主单个 buffer 上限为 2^32-1 字节（byteLength 以 u32 编码），更大长度按分配失败 `RangeError` 处理，而 V8 的 kMaxByteLength 可更高。
