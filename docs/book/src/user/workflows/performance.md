...
静态模块在执行前编成 generic native，入口没有解释器预热；这不等于执行路径永远平坦，也不等于比 V8 更快。generic 代码仍走动态语义；热路径 overlay 会在反馈稳定后额外编译，可能出现编译与 deopt 尖刺。`eval` 每次都要付编译成本。性能调优的杠杆是堆预算、磁盘缓存和编译观察。生产 GC 固定为并发分代 ZGC，暂停时间与堆大小基本无关。
