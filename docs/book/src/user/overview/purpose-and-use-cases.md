...
- **无 JIT**。不依赖运行时 profile 做热点优化，首屏性能即稳态性能。

## AOT 的价值

| 属性 | 含义 |
| --- | --- |
| 启动快 | 无解释预热、无 JIT 编译开销，入口函数从第一条指令就开始执行 |
| 无 JIT 抖动 | 没有 deopt、recompile、on-stack replacement 带来的延迟尖刺 |
...
| JIT 热点优化 | 当前架构无 profiling JIT；长时间运行且依赖热点优化的负载不会有同性能 |
