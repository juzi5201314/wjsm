...
| `JSON` | 可用 | |
| TypedArray、`ArrayBuffer`、`SharedArrayBuffer` | 可用 | 实例原型链完整挂接（`instanceof` / `constructor` / `@@toStringTag` 品牌一致），原型方法可取值并经 `call`/`apply`/`bind` 复用，`byteLength` 等规范 accessor getter 在位；`ArrayBuffer` resizable / transfer 家族（`resize` / `transfer` / `transferToFixedLength` / `resizable` / `maxByteLength` / `detached`，ES2024）完整可用，TypedArray / DataView length-tracking 视图随底层改长重算；`SharedArrayBuffer` 是全局对象上真实自有属性；TypedArray `toLocaleString` 委托与 Array 相同的 Intl 路径 |
