...
packed exe 离开构建机源码树后仍可在同 OS/arch 上运行：解析、JSON、动态 `import` / `require`、`new Worker` 与 `__dirname` 下的 `fs` 读只走快照。`import.meta` 与 `process.argv[1]` 使用虚拟身份 `/wjsm-exec/...`，不含构建机路径。`cluster.fork` / `child_process.fork` 再执行同一个 exe。快照 = 打包期实际读过的文件、静态 Worker/fork 相对路径，加上 `--include`；未命中则 fail-closed，不回退读盘。写虚拟路径失败。相对 `cwd` 的文件名不是快照。`--include` 只属于 native-executable，越界或缺文件则打包失败且不写输出。portable `.wjsm` 不带快照。`wjsm run` 仍读真实磁盘。
