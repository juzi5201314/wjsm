...
| codegen | `dump-wat` / `disasm` | WAT / 反汇编 |
| host/runtime | fixture + 运行 | stdout/stderr |

比较相邻阶段的输出：如果 `dump-ast` 正确但 `dump-ir` 错误，问题在 lowering；如果 `dump-ir` 正确但 `dump-wat` 错误，问题在 codegen。

## 禁止临时日志

AGENTS.md 要求：不要在生产代码加临时日志，用 `dump-ast`、`dump-ir`、`dump-wat`、`disasm` 替代。如果这些工具无法暴露问题，再考虑其他手段。
