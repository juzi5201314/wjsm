...
**单一 owner。** 每个事实只有一个权威定义点。GC 算法解析在 `NativeRuntimeConfig::from_environment`，影子栈默认值在 `wjsm_ir::SHADOW_STACK_DEFAULT_MAX_SIZE`，缓存目录由调用方传入的 `cache_dir` / `WJSM_CACHE_DIR` 决定（默认关闭）。其他位置引用，不复制。
