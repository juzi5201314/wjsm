...
>
> wjsm 没有 AST 解释器或字节码 VM。eval 走的编译路径和静态模块完全一致——解析、lowering、CLIF、native image——只是触发时机在运行时，因此是动态编译，不是执行前 AOT。
>
> 动态代码和静态模块共用同一套 native 执行路径，没有「解释器语义 vs JIT 语义」两套结果。代价是每次未命中缓存的 eval 都有编译延迟。
>
