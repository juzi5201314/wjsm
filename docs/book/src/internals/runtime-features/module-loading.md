...
运行时如何加载和执行用户模块，取决于模块的类型和来源。

## 模块来源

| 来源 | 处理方式 |
| --- | --- |
| 主模块（编译期） | 由 `wjsm-module` 解析、链接、bundle 成单个 IR Program，运行时 `compile_source` 绑定入口 |
| 动态 `import()` | 运行时解析 specifier、加载源码、编译、实例化 |
| `node:vm` 构造器 | 编译期未知的代码，走 eval 模式编译 |
