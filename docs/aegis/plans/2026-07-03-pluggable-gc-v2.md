...
**ADR Signal**（保留至完成回填）: INV-C 重写、GcAlgorithm v2 边界、三变体物理边界、增量调度决策 → `docs/adr/0005-pluggable-gc-v2.md`（P6）。baseline-sync：AGENTS.md/CLAUDE.md WASM contract（globals 20→29，删除旧 `__alloc_counter` + 新增 10、host funcs 变更）与 GC 描述。
...
**Files**: `compiler_module/module_compile.rs`（v2 起 `object_heap_start` 64KB 对齐，shadow guard 后 padding 不参与对象堆）、`wjsm-snapshot-format/src/lib.rs`（format 版本 +1；字段重命名/新增 `immortal_objects_end_rel: u32`；`abi_hash()` 输入追加 v2 布局常量：REGION_SIZE=65536/CARD_SIZE=512/globals 并集名单/flavor-independent support ABI union hash）、`wjsm-runtime/src/runtime_startup.rs`（restore 后 `RuntimeState.immortal_objects_end` = 快照有效对象末端，`RuntimeState.dynamic_start = align_up(immortal_objects_end, 64KB)`；冷启动 = bootstrap 完成后当场划定）、`lib.rs`（`immortal_objects_end`/`dynamic_start` 字段 + `attach_heap` 调用点：实例化完成后对当前算法调 `attach_heap(ctx, dynamic_start)`）。
...
**Files**: `compiler_core.rs` / `compiler_module/module_compile.rs` / `compiler_module/module_setup.rs`（删除 `__alloc_counter`，新增/re-index/re-export `__alloc_ptr __alloc_end __gc_alloc_bytes __gc_trigger_bytes __gc_phase __good_color __card_table_base __region_meta_base __satb_ptr __satb_end`，语义表 = spec §7.3）、`wjsm-runtime/src/wasm_env.rs`（对应 `Option<Global>` 字段 + `from_caller` 解析）、`wjsm-runtime/src/runtime_startup.rs`（linker/shared env globals 创建 29 个）、`support_module.rs`（同名 import 对齐）、`wjsm-runtime-support/src/abi.rs`（ENV_GLOBALS/ABI hash 输入同步）。
**三变体一致性约束**（IMPL-19 单源保证）：现有 20 个 env globals 中删除旧 `__alloc_counter`，新增 10 个 v2 globals，合计 29 个；所有 10 个新增 globals 在三变体的 `module_setup.rs` + `support_module.rs` 中**全部声明**（import/re-export），保证 `WasmEnv::from_caller` 解析时字段集一致。mark-sweep 变体的 WASM 代码不读写 `__good_color`/`__card_table_base`/`__region_meta_base`/`__satb_*`（保持初始值），仅在 P3/P4 emitter 变体臂中生成对应指令。
**Verification**: dump-wat 检查 globals 段（三变体均删除 `__alloc_counter`，并含保留的既有 19 个 + 新增 10 个 globals 声明，合计 29）；全量绿（新增 globals 未被读写，纯扩容约定）。
...
pub fn write_property_slot(ctx, env, h, slot_idx, part: SlotPart, val: i64);  // 读旧 i64 值 → on_host_write → 写
pub fn write_element(ctx, env, h, idx, val: i64);                            // 读旧 i64 值 → on_host_write → 写
pub fn write_proto(ctx, env, h, proto: u32);                                 // proto u32/null 哨兵先转 NaN-boxed old/new value 再 barrier
```
...
**内容**: `RegionMeta` 枚举（`Free/Eden/Survivor/Old/HumongousStart/HumongousCont/Immortal/Meta`，u8 编码与 WASM 侧一致）；`attach_heap`：计算 `gc_reserved_end = heap_limit`（配置 heap budget）或 wasm32 地址上限（未配置时 metadata 最坏约 8.1MiB）→ 元数据区（region_meta 表 + card table + SATB 4KB）一次性保留，必要时先 grow 到 metadata 末端 → 写 `__region_meta_base/__card_table_base/__satb_ptr/__satb_end` globals → region 域 64KB 对齐起点 → immortal 段标注 → 首个 Eden region 设窗口。**索引基准固定为 `object_heap_start`**：`region_idx = (addr - object_heap_start) >> 16`，`card_idx = (addr - object_heap_start) >> 9`；元数据覆盖 `[object_heap_start, gc_reserved_end)`，不是只覆盖动态 region 域。region 分配/归还 API（`take_free_as(kind)` / `release(idx)`）。host 分配路径（`alloc_host_object_impl`）接 `alloc_slow`。
**Verification**: 单测（域划分边界/humongous 连续段/immortal 标注/card 与 region 索引基准/未配置 heap limit 时 metadata sizing <= 约 8.1MiB 且 attach 先 grow metadata）；`WJSM_TEST_GC=g1 cargo nextest run -E 'test(happy__hello)'` 冒烟（registry 本任务起对 G1 返回 Ok）。

### T3.2 card.rs + host 侧 barrier

**Files**: 新建 `g1/card.rs`；`heap_access.rs`（`on_host_write` 分派已就位，G1 impl 补 Rust 版 SATB+card，逻辑 = spec §8.2 双 (a)(b)）。SATB entry 必须存旧槽位的 **i64 NaN-boxed Value**（8B），不是 ptr/handle；flush 时统一用 roots/value 解析逻辑转 handle，drain 后必须把 `__satb_ptr` 重置到算法记录的 SATB base。card index 必须以 `object_heap_start` 为基准。
**Verification**: 单测（card 索引计算/dirty 迭代器/SATB 队列 8B entry flush/value→handle 解析/flush 后指针复位/满 4KB 后继续写）。

### T3.3 g1 变体 barrier 代码生成 + 第二份 cwasm

**Files**: `support_module.rs`/`support_object_helpers.rs`（`GcFlavor::G1` 臂：`obj_set`/`elem_set` 及一切引用槽写入点插入 spec §8.2 指令序列；SATB 写旧 `i64 Value` 并按 8B 推进；card 以 `object_heap_start` 为基准；**G1 allocate-black 定案为 region-level implicit-black**：mark 期 host 换出的 Eden/Survivor/Old/Humongous region 或 evacuation/promotion 目的 region 标记为本周期全活，cleanup 不回收，WASM 分配序列零额外 mark bitmap 指令，不新增 `__mark_bitmap_base`）、`wjsm-runtime-support/build.rs`（+`wjsm_support_g1.cwasm`）、`lib.rs`（install 按 kind 选变体）、`runtime_eval.rs`（eval flavor 传递）。
**Verification**: dump-wat（g1 变体 SATB 8B/value barrier + card 序列）；`WJSM_TEST_GC=g1` 跑 `happy__` 子集冒烟。

### T3.4 young.rs（young GC）

**Files**: 新建 `g1/young.rs`；`runtime_gc/roots.rs`（新增共享函数 `for_each_immortal_region_root(ctx, visit)`：遍历 live `obj_table` handles，筛选 ptr 落在 `[object_heap_start, immortal_objects_end)` 的对象并扫描引用槽——P4 zgc mark 复用，spec §14）。immortal 扫描必须复用统一对象 walker；非法 header 是 runtime invariant 破坏，debug 断言/release trap，不允许线性扫描 padding 或 abandoned 旧块。
**内容**: spec §10.3 全流程：root 集（复用 `RuntimeRoots` + immortal 对象扫描 + dirty card 精化扫描）→ young 对象图复制（Survivor bump / age≥2 晋升 Old / Survivor 不足直晋升）→ obj_table 更新 → freed handles → weak refs 共享处理 → region 归还 → GcStats（cycle_kind=Young, pause 记录）。dirty card 扫描后按精化结果处理：card 不再含 young 引用则清 dirty；仍含 Eden/Survivor 引用则保持/重新置 dirty，防 old→survivor 引用下一轮漏根。复制/晋升到 Old/Humongous 后必须扫描目的对象槽位，若仍含 young 引用则标脏目的 card。**复制期间 INV-C2**：young GC 在 host call 内 STW，复制序列自身不触发分配路径以外的 GC 点（复制目标 region 由算法直接管理，不走 `alloc_slow`）。
**Verification**: 单测（age 晋升/survivor 溢出/humongous 不动/dirty card re-dirty/晋升目的 card re-dirty/immortal 扫描跳过 padding 与 abandoned）；新 fixture `gc_g1_young_churn.js`（分配-丢弃循环 + 存活集校验 stdout）。

### T3.5 concurrent_mark.rs（增量标记）

**Files**: 新建 `g1/concurrent_mark.rs`。
**内容**: spec §10.4：IHOP 触发（old 占 45%）→ 初始标记附着 young GC，先执行完整 `RuntimeRoots` fixed-point snapshot（含 host 侧表）并在 mutator 恢复前入队/标记 → `safepoint_step` 增量 drain（budget 切片；SATB 缓冲并入）→ final remark（STW：SATB 残留 + `RuntimeRoots` fixed-point 重扫 = IMPL-18）→ cleanup（region 活字节统计 + 全死 region 归还）。mark bitmap 复用 `MarkBitmap`（handle 索引）。G1 allocate-black 使用 region-level implicit-black：mark 期新发放或作为复制/晋升/evacuation 目的地的 region 本周期视为全活，cleanup 跳过，下一轮再精确统计。
**Verification**: 单测（SATB 一致性场景：标记中覆盖写 old 引用，对象存活；mark 中删除 host side-table 唯一旧 root 后对象仍存活到本周期结束；mark 期新分配/复制目的 region 不被 cleanup 回收）；`WJSM_TEST_GC=g1` 长循环 fixture。

### T3.6 mixed.rs（CSet evacuation）

**Files**: 新建 `g1/mixed.rs`。
**内容**: spec §10.5：活字节升序选 CSet（pause budget 折算字节上限；>85% 活跳过）→ STW evacuate（old→old 复制 + obj_table 更新 + freed handles）→ 多轮直至候选耗尽。wjsm 引用槽保存 handle，CSet evacuation **不做 per-reference 修正，也不需要 incoming-reference remembered set**；更新 `obj_table[h]` 即全堆生效。复制到 Old/Humongous 后同 T3.4 扫描目的对象槽位，若仍含 young 引用则标脏目的 card。
**Verification**: 单测（CSet 预算截断/85% 阈值/evacuate 后引用槽无需改写仍读到新对象/evacuate 目的 card re-dirty）；碎片 fixture（`gc_fragmentation_churn` @ g1 断言 `external_fragmentation` 下降——经 GcStats 可观测输出）。
...
**内容**: 色协议常量与 entry 读写（`entry = ptr | color`，掩码 `0x3`/`!0x3`）：`00 = Empty/Free`，`01 = Marked0`，`10 = Marked1`，`11 = Remapped`；双 good 切换状态（spec §11.2）；page 域组织（attach_heap 同 T3.1 模式，无代别，page meta 覆盖 `[object_heap_start, gc_reserved_end)` 并一次性保留）。ZGC attach/restore/cold freeze 后必须把所有 live `obj_table[h]` recolor 为 `ptr | initial_good_color`，运行期 live entry 不允许保持 `00`。
**Verification**: 色协议单测（双 good 切换全状态转移表；attach 后 live entry 非 00；relocate 期 `good=11`；非 RS 坏色 entry 修复为 `ptr|11`）。
...
**Files**: `api.rs`（spec §17 字段全量，含 `relocated_objects`、`load_barrier_mark_hits`、`load_barrier_relocate_hits`）、`lib.rs`（`gc_pause_hist` 环形缓冲 256 条）、各算法填充点、`WJSM_GC_LOG=1` 周期摘要 eprintln。
...
**Files**: `AGENTS.md`/`CLAUDE.md`（WASM contract：globals 20→29，删除旧 `__alloc_counter` + 新增 10、host funcs 表、GC 描述改三算法可选）、`docs/aegis/specs/2026-07-03-napi-native-addon-design.md`（两处 "non-moving" → "handle 恒定（INV-C1）"，spec §15.2）。
...
| `gc_maybe_collect`（import + 每分配 host call）+ 旧 `__alloc_counter` global | T1.4 / T1.2 | grep 零命中；dump-wat globals 合计 29 |
