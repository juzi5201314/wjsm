...
```
freed handles 入 `handle_free_list`、weak refs、owner-backed 侧表回收协议原样保留在周期收尾。sweeper 新增 `sweep_cursor` + block epoch；未扫 block 不进入 free list，已扫 free runs 仍按地址序入表；`alloc_slow` 在 free list 不足时先 sweep 若干 block 偿还 allocation debt，再考虑 grow。
...
**内容**: `RegionMeta` 枚举（`Free/Eden/Survivor/Old/HumongousStart/HumongousCont/Immortal/Meta`，u8 编码仅 host-side）；`attach_heap`：dynamic_start 起直接划分 64KB region 域 → 初始化 host-side `region_meta`（含 region age / implicit-black epoch）+ sparse remembered set → immortal 段标注 → 首个 Eden region 设窗口。**索引基准固定为 `object_heap_start`**：`region_idx = (addr - object_heap_start) >> 16`，`card_idx = (addr - object_heap_start) >> 9`；metadata 覆盖 committed region 域 + immortal 段，grow 时追加，不占用 wasm dynamic heap。region 分配/归还 API（`take_free_as(kind)` / `release(idx)`）。host 分配路径（`alloc_host_object_impl`）接 `alloc_slow`。
**Verification**: 单测（域划分边界/humongous 连续段/immortal 标注/card 与 region 索引基准/grow 后 metadata 扩展/hello 不预留 8MiB card table）；`WJSM_TEST_GC=g1 cargo nextest run -E 'test(happy__hello)'` 冒烟（registry 本任务起对 G1 返回 Ok）。

### T3.2 rset.rs + host 侧 barrier

**Files**: 新建 `g1/rset.rs`；`heap_access.rs`（`on_host_write(target, slot_addr, old_val, new_val)` 分派已就位，G1 impl 补 Rust 版 SATB+RSet，逻辑 = spec §8.2）。barrier event entry 必须是 24B `{flags:u32, slot_addr:u32, old_value:i64, new_value:i64}`；SATB 存旧槽位的 **i64 NaN-boxed Value**，不是 ptr/handle；RSet event 使用 event 自带 `new_value`，flush 不依赖读取当前 slot，避免写前满缓冲 flush 漏 old→young；flush 时统一用 roots/value 解析逻辑转 handle；默认 sparse dirty-card set，热点 card 可升级 precise-slot bitmap。flush 后必须把 `__barrier_buf_ptr` 重置到 runtime 记录的 buffer base。
**Verification**: 单测（card 索引计算/sparse dirty 迭代器/热点 precise-slot 升级/SATB value→handle 解析/event buffer 24B 编码/flush 后指针复位/满 24KB 边界 old→young 写不漏 RSet/host 写 slot_addr 精确标 card）。
...
**内容**: spec §10.3 全流程：root 集（复用 `RuntimeRoots` + immortal 对象扫描 + host-side dirty card / precise-slot 精化扫描）→ young 对象图复制（Survivor bump / age≥2 晋升 Old / Survivor 不足直晋升）→ obj_table 更新 → freed handles → weak refs 共享处理 → region 归还 → GcStats（cycle_kind=Young, pause 记录）。dirty card 扫描后按精化结果处理：card 不再含 young 引用则清 dirty；仍含 Eden/Survivor 引用则保持/重新置 dirty，防 old→survivor 引用下一轮漏根。复制/晋升到 Old/Humongous 后必须扫描目的对象槽位，若仍含 young 引用则标脏目的 card。**复制期间 INV-C2**：young GC 在 host call 内 STW，复制序列自身不触发分配路径以外的 GC 点（复制目标 region 由算法直接管理，不走 `alloc_slow`）。
**Verification**: 单测（age 晋升/survivor 溢出/humongous 不动/dirty card re-dirty/晋升目的 card re-dirty/immortal 扫描跳过 padding 与 abandoned/oblet 拆分不漏槽）；新 fixture `gc_g1_young_churn.js`（分配-丢弃循环 + 存活集校验 stdout）。
...
**Verification**: 色协议单测（双 good 切换全状态转移表；attach 后 live entry 非 00；host-side page meta grow；relocate 期 `good=11`；非 RS 坏色 entry 修复为 `ptr|11`）。
