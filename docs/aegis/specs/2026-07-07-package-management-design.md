...
- 日期：2026-07-07
- 状态：待用户审阅
- 关联：CLAUDE.md AOT 架构约束、issue #311（Phase 4 生态/CLI tooling）、issue #312（runtime module loading，共享 registry/loader 边界）、issue #309/#310（package resolution / node globals 现有代码）
...
更进一步：因为 wjsm 拥有编译器，它能做到 npm/pnpm/bun 都做不到的事——**跨项目缓存"包 → 编译产物"**。同一个 `lodash@4.17.21` 在整台机器上只需被 parse/lower/编译一次，任何新项目 install 后直接复用编译产物。这是 AOT 包管理器相对所有 JS 包管理器的结构性差异化优势。
...
| 编译产物缓存 | **做，分层（L0 源码 / L1 IR / L2 cwasm）** |
...
- **L1 IR 片段**：单个包模块 parse + lower 后的 IR（`wjsm-ir` 片段），按内容寻址缓存。跨项目复用——避免每个项目重复 parse/lower 相同的包。
- **L2 cwasm**：编译后的 WebAssembly 产物，按内容寻址缓存。复用现有 `runtime_startup.rs` 的 `compile_or_load_cached`（wasmtime `Module::deserialize_file`）机制的思路，扩展到包粒度。

### 8.2 缓存 key

```
L1 key = blake3(manifest_hash ‖ semantic_compiler_version ‖ module_lowering_flags)
L2 key = blake3(L1_key ‖ backend_abi_hash ‖ gc_flavor)
```

- `backend_abi_hash` / `gc_flavor` 复用 `wjsm-snapshot-format::abi_hash` 与既有 GC 选择常量，任一变更自动失效缓存（回退重编译）。
- 编译器版本变更 → key 变更 → 自动失效，不需手动清缓存。

### 8.3 分阶段可行性（诚实标注）

§8.1/§8.2 描述的是**目标态**（per-package L1/L2）。落地分两粒度，避免歧义：

- **首版：L0 + L2-bundle（整 bundle 粒度）**。key = `blake3(lockfile 解析图 ‖ compiler_ver ‖ backend_abi_hash ‖ gc_flavor)`，**不含 L1_key**——因为首版仍是整程序 bundle 编译，产物按整个解析图寻址。直接复用现有 `runtime_startup.rs` cwasm 缓存机制。同一 lockfile + 同编译器 → 跳过整个编译，覆盖"CI 重复构建"与"同项目多次 run"。
- **Phase 2：L1 + L2-pkg（包粒度）**。§8.2 的 `L2 key = blake3(L1_key ‖ ...)` 是此阶段目标，实现真正的**跨项目包级复用**，依赖**分离编译**（把单个包 lower/编译成可复用片段再链接）。wjsm 现为整程序 bundle 编译，分离编译是一次架构演进，与 issue #312 的 multi-instance shared-env loader 相关。**首版不阻塞**；planning 阶段需先验证 `wjsm-semantic::lower_modules` 的模块间耦合度（scope id 是否模块局部可复用）。此阶段建议拆为独立 issue。
...
4. 分层编译产物缓存与分离编译演进路径（与 issue #312 loader 边界的关系）。
...
- `artifact`：L2 key 随 compiler_ver/abi 变更失效。
...
- Target：新 crate `wjsm-pm`（solver/registry/store/lockfile/scripts/workspace 六大子模块）；`wjsm-module` 新增 `Vfs`/`ResolutionOverlay` trait 并把三处磁盘访问改为 trait 调用；`wjsm-cli` 新增 install/add/remove/task/x 子命令组装。
- Current pressure：`resolver.rs` 已 1576 行、`cjs_transform.rs` 984 行、runtime `lib.rs` 2115 行——已超纪律，禁止再往这些大文件塞包管理逻辑。
- Planned governance：所有新逻辑进 `wjsm-pm` 独立 owner 文件（每文件单一职责 ≤500 行）；`wjsm-module` 仅做 trait 抽象的微创手术（把 3 处 `fs::` 调用改为 trait 调用 + 新增 trait 定义文件），不改解析算法。
...
- **P5 编译产物缓存**：L0+L2 whole-bundle 缓存落地；L1 per-package 分离编译标注为独立后续 issue（依赖 #312 演进）。
...
- Non-goals：见 §3。
- Risks：PubGrub 与 npm 嵌套重复语义的调和正确性；分离编译（L1）可行性；无 node_modules 对第三方工具链的兼容代价；SQLite/packfile 并发写正确性。
