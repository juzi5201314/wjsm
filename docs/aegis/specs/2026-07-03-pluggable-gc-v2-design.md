...
pub trait GcAlgorithm: Send + Sync {
    ...
    /// immortal 区 [object_heap_start, dynamic_start) 由框架维护，算法只读、
    /// 不回收、不搬迁（§15.1）。算法在此划元数据区（card table/region meta/
    /// SATB buf）、初始化分配窗口 globals（__alloc_ptr/__alloc_end）。
...
[object_heap_start → immortal 区（snapshot 对象，§15.1）→ __immortal_end]
[dynamic heap 域（算法自治）→ heap_limit]
```

- mark-sweep：dynamic 域 = 连续 bump + free list（现状延续，D6）。
- g1/zgc：dynamic 域头部 = 算法元数据区（§10.1/§11.1），其后 = region 数组（64KB/region，与 wasm 页对齐）。
...
;; g1/zgc 变体追加：mark 期 allocate-black（新对象置已标记/good 色，§10.4/§11.3）
```
...
### 7.3 globals 清单（v2 全集 = 现有 19 + 新增 9）
...
| `__card_table_base` | i32 imm | host attach_heap | WASM write barrier | g1 card table 基址 |
| `__region_meta_base` | i32 imm | host attach_heap | WASM write barrier | region 元数据表基址（1B/region） |
...
| `gc_barrier_flush()` | 新增；SATB 缓冲满 → `barrier_flush` |
...
所有 helper emit 函数接 `flavor`，差异点以 `match flavor` 局部分支表达；**不允许**为变体复制整份 emitter（drift 风险 R4）。`support_module_layout_hash()` 对三变体分别计算，`wjsm-runtime-support` build.rs 产出 `wjsm_support_{mark_sweep,g1,zgc}.cwasm` 三份并全部嵌入；`install_embedded_support_cwasm` 按选定算法装载。
...
resize 序列（`$obj_set` 扩容、`$arr_push` 扩容、arguments 物化等所有"分配后引用旧数据"的 helper）改为：`gc_alloc_slow`/分配返回后**重新 `obj_table[h]` 解 old_ptr** 再 `memory.copy`。解引用一律经 `emit_resolve_handle_ptr(flavor)`（zgc 下自动含 load barrier）。P2 产出完整违反点清单文档核对（R1 缓解）。
...
```
[dynamic_start → 元数据区：region_meta（1B/region，覆盖 object_heap_start..heap_limit）
                + card table（1B/512B card，同覆盖范围）
                + SATB 缓冲（4KB）]
[region 域：N × 64KB region，域起点 64KB 对齐]
```

region_meta 值：`FREE/EDEN/SURVIVOR/OLD/HUMONGOUS_START/HUMONGOUS_CONT/IMMORTAL/META`。immortal 段与元数据段在表中标注（barrier 与扫描统一按表判定）。大对象（size > 32KB = region/2）→ 连续 humongous region，直入 old 逻辑代。
...
roots = shadow stack + host 侧表（全扫，条目数量级小）+ immortal 区引用槽扫描 + **dirty cards** 指出的 old/humongous 对象中的引用槽（扫后清 card，指向 young 的精化在此发生）。
遍历只 follow young 对象（old 活性由并发标记负责）；活对象复制到 Survivor region（age+1；age ≥ 2 或 Survivor 不足 → 晋升 Old）；**只更新 obj_table**（INV-C1 红利：零引用修正）；死亡 young 对象 handle 回收进 free list；Eden/被清空的 From-Survivor region 归还 Free。weak refs 按本轮 freed handles 处理（共享层）。

### 10.4 并发标记（增量切片）

触发：old（含 humongous）占 heap_limit 比例 ≥ IHOP（45%）。
- **初始标记**（STW，附着在一次 young GC 上）：root snapshot，`__gc_phase = MARK`；
- **增量 drain**：`safepoint_step` 按 budget 处理 worklist（全堆 mark，覆盖 young+old；SATB 缓冲定期并入 worklist）；mutator 侧 SATB barrier（§8.2a）维持快照不变式；**mark 期新分配对象 allocate-black**（分配序列直接置标记位）；
...
标记完成后：按 region 活字节升序选 CSet（pause budget 折算复制字节上限），STW evacuate（old→old 压缩复制 + obj_table 更新）；分多次 mixed 直到候选耗尽或收益低于阈值（活字节 > 85% 的 region 不搬）。碎片由此根治（#332 目标的 g1 解）。
...
`[dynamic_start → 元数据区：region(page) meta + SATB/mark 缓冲][page 域：N × 64KB zPage]`。非分代（D10）。
...
- P2 产出机械核对清单：`grep` 全部 `HEAP_OBJECT_PROPERTY`/`HEAP_ARRAY_ELEMENT` 偏移写点逐条勾销（R1）。
- host 侧表写入（promise/continuation 等 Rust 结构）**不经此层**（不是堆写）——一致性由 final remark 重扫兜底（§10.4/§11.3）。
...
- 新增共享扫描项：**immortal 区引用槽扫描**（mark 与 young GC 的 root 源，§10.3/§11.3）。
...
快照堆映像 = **bootstrap immortal 区**：restore 后 `[object_heap_start, __immortal_end)` 为三算法一致的永久区（primordial 对象本就永久可达）——不回收、不搬迁、参与 root 扫描与 card 覆盖。动态堆从 `__immortal_end`（64KB 对齐上取）开始由算法 `attach_heap` 接管。snapshot format 新增 immortal 边界字段 + format 版本递增；`abi_hash()` 输入追加：GcFlavor 无关的 v2 布局常量（card/region 参数、globals 并集签名）。冷启动路径（无快照）：bootstrap 完成后当场划定 immortal 边界，行为一致。
...
pub struct GcStats {
    ...
    pub load_barrier_slow_hits: usize,
}
```
...
| `gc_maybe_collect` host import + `$obj_new`/`$arr_new` 头部无条件调用 | **删除**（§7.2 counter 内联 + `gc_safepoint_poll`） |
...
| **R2** host 侧表结构变化不经 barrier → SATB 不变式破洞 | final remark STW fixed-point 重扫兜底（§10.4/§11.3）；侧表条目量级小，remark 成本可控 |
| **R3** zgc RELOCATE 期 host 写旧位置 | `heap_access::resolve` 强制 `on_host_resolve` heal；debug 断言"RS 内对象解引用必已 heal"；P4 专项测试 |
| **R4** 三变体 emitter drift | 单源参数化（§8.1 禁复制）；三变体 layout hash 独立入 ABI；共用指令序列测试 |
...
| **R7** snapshot 兼容回归 | immortal 区设计不改快照对象本身；format 版本 + ABI hash 既有 fallback 机制；P1 冷/热双路径测试 |
...
| **IMPL-14** | mark 期 allocate-black：g1/zgc 变体分配序列直接置标记/good 色 |
