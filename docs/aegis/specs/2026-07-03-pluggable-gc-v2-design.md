...
pub trait GcAlgorithm: Send + Sync {
    ...
    /// 本轮回收释放的 handle 列表（供 weak/side-table cleanup 与 handle_free_list 复用，IMPL-10 继承）。
    /// 算法在周期收尾时先把完整 freed 集交给共享 cleanup，再经 ctx.with_state 推入 handle_free_list（协议同 §14）。
...
;; header 初始化 + obj_table 注册（INV-A 继承）：MarkSweep/G1 写裸 ptr；ZGC 直接写 `ptr | __good_color`。
;; G1 allocate-black 只更新 host-side region implicit-black epoch；ZGC allocate-black 由有色 obj_table entry 表达（§10.4/§11.3）。
```
...
| `gc_barrier_flush()` | 新增；drain 统一 barrier event buffer：host import 读取 event 后调用当前算法 `barrier_flush(ctx)` 处理 SATB/RSet 队列；SATB event 按 `old_value` 入当前 mark worklist，RSet event 仅 G1 使用 event 自带的 `slot_addr/new_value` + host-side metadata 过滤/去重；最后把 `__barrier_buf_ptr` 重置为 buffer base。**硬约束：不得 collect/grow/move/recolor，不递增 `gc_epoch`，不是 INV-C2 GC 点** |
...
保守性：WASM 不查 owner region，也不查 new_val 是否 young；所有由 WASM barrier 记录的引用槽必须是线性内存对象 payload/header 内的 inline slot，host flush 可用 `slot_addr` 反查 owner region/card，并用 event 自带 `new_value` + `obj_table` + host-side `RegionMeta` 精化，只有 old/immortal/humongous→young 的槽进入 RSet。若未来引入 out-of-line 线性存储，必须改走 host `on_host_write(target, slot_addr, old, new)` 或扩展 event owner handle，禁止只塞无法反查 owner 的裸地址。这样牺牲少量 buffer 带宽，换掉线性内存 card table/region table 和 barrier 内 metadata 访问，同时避免“写前 flush 读到旧槽值”漏记 RSet。RSet 在 host 侧采用 sparse dirty-card set；热点 card 可升级为 precise-slot bitmap（实现可选，但接口预留）。
...
遍历只 follow young 对象（old 活性由并发标记负责）；活对象复制到 Survivor region（age+1；age ≥ 2 或 Survivor 不足 → 晋升 Old）；**只更新 obj_table**（INV-C1 红利：零引用修正）；死亡 young 对象先汇总为本轮 freed handles，按 §14 完成 weak/side-table cleanup 与 finalization 入队后，再发布到 handle_free_list；Eden/被清空的 From-Survivor region 归还 Free。复制/晋升到 Old/Humongous 后必须扫描目的对象槽位：若仍含 Eden/Survivor 引用，则标脏目的 card，防复制后 old→young 边丢失。
...
- **cleanup**：统计各 region 活字节；按 mark bitmap 汇总 old/humongous dead handles，先执行 §14 weak/side-table cleanup 与 finalization 入队，再发布到 handle_free_list；全死 region 直接归还 Free；implicit-black region 本周期跳过回收（视为全活），下一轮 mark 再精确统计。

### 10.5 mixed GC

标记完成后：按 region 活字节升序选 CSet（pause budget 折算复制字节上限），STW evacuate（old→old 复制 + obj_table 更新）；**不做 per-reference 修正，也不需要 incoming-reference remembered set**：引用槽保存 handle，`obj_table[h]` 更新后全堆自然指向新位置。复制到 Old/Humongous 目的 region 后同 young 晋升规则扫描目的对象槽位，若仍含 young 引用则标脏目的 card。mixed 分多次执行直到候选耗尽或收益低于阈值（活字节 > 85% 的 region 不搬）。dead handles 已在 concurrent-mark cleanup 中按 §14 统一清理并发布；mixed 只回收/压缩空间，不再次发布 handles。碎片由此根治（#332 目标的 g1 解）。
...
Idle → MarkStart(STW: __good_color=本周期 mark 色(双色交替), root snapshot,
     ...
     → MarkEnd(STW: SATB 残留 + host 侧表 fixed-point 重扫；按 mark bitmap 汇总全堆 dead_handle_set，先执行 weak/side-table cleanup 与 finalization 入队，再把 dead handles 发布到 handle_free_list)
     → SelectRelocSet(活字节/碎片率排序；live_bytes=0 的 page 可立即回收；其余优先低密度/高碎片 page，dense page aging-in-place；碎片率>25% 的 page 候选，按预算截断)
     ...
     → Idle(收尾: stats 写入；未选入 RS 的 page 仅保留不可复用垃圾字节，不保留已死亡 handle；后续 relocation/page 回收只处理空间，不再次发布 handles)
```
...
- weak refs / stream 清理 / owner-backed 侧表回收：v1 协议保留，入口改为“算法周期收尾统一调用共享函数”（`weak_refs::process_after_collection(ctx, freed)`）。**硬约束**：所有算法必须先用本轮完整 `freed` handle 集完成 weak/side-table cleanup 与 finalization 入队，再把这些 handles 发布到 `handle_free_list`；ZGC 这类“标记时确定死亡、搬迁时才回收空间”的算法也不得在 cleanup 之前复用 handle。
