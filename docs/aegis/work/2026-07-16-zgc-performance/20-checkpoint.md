...
Task 14 已完成并通过 feature/default realm gate；当前进入 Task 15（原子 active ABI cutover audit）。
...
Task 15：生成 cutover audit，确认 Tasks 0–14 和 pre-cutover gates 后以单一 ABI 切换删除 private gate/旧 dynamic heap 主路径。
...
- 先运行 Task 15 的 workspace/all-features、三种 GC happy fixture pre-cutover gates；再生成逐 caller cutover audit，任何旧 active owner 或 gate 失败都必须保持 RED。
- 仅在 audit RED 证据完整后，按单一原子任务迁移所有 active caller；禁止按 collector 或 consumer 分裂 active ABI。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–14 段、父规格与实施计划；Task 14 已实现并完成验证，下一项为 Task 15 cutover audit。

## DriftCheckDraft

- Scope：Task 14 新增 V2 realm remap 和 conditional-root adapter；shared `HandleTableV2` 是唯一 handle truth，realm/Promise/stream/proxy/async values 只在 handle live 时作为 V2 root。
- Compatibility：default Realm、node:vm clone、legacy root scanner 和 side-table owner 未切换；V2 adapter 不建 collector、worker 或 epoch copy，留待 Task 15 统一接入。
- Retirement：legacy realm/main-memory path 仍为 default main path，Task 15/26 原子切换/退役；不存在 feature runtime fallback。
- Decision：Task 14 GREEN，进入 Task 15。
