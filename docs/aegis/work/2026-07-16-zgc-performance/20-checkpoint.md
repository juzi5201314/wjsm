...
Task 13 已完成并通过 V2/active V1 artifact gate；当前进入 Task 14（realm、handle-remap 与 side-table 迁移准备）。
...
Task 14：准备 feature-gated realm、handle-remap 与 side-table 迁移，保持 default realm owner 不切换。
...
- 从 `handle_remap.rs`、realm clone、VM roots 与 Promise/stream/proxy/async side-table owner 开始 Task 14，复用唯一 V2 heap/handle/epoch owner。
- 保持 Task 10–13 的 V2 gates 与默认 realm/runtime gates 同时通过。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–13 段、父规格与实施计划；Task 13 已实现并完成验证，下一项为 Task 14。

## DriftCheckDraft

- Scope：Task 13 新增独立 V2 snapshot wire ABI 和 feature-gated artifact manifest；page ranges、bitmap counts、8-byte raw handle entries/generation、engine fingerprint 和 managed support ABI 都在格式边界验证。
- Compatibility：active V1 `SNAPSHOT_FORMAT_VERSION` 仍为 9，legacy startup snapshot encode/restore 和默认 support cwasm 未切换；V2 artifact 仅在 `wjsm-runtime-snapshot` 的 private feature 下构建/嵌入。
- Retirement：没有启用第二条 active startup path；legacy V1 snapshot artifact 保持 main path，Task 15/26 才执行原子切换与退役。
- Decision：Task 13 GREEN，进入 Task 14。
