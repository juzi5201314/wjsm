...
Task 7：实现固定 GC worker pool、packet slab/work-steal/termination，并分离 loom/Miri/TSan 模型；仍在私有 feature gate 下。
...
- 读取现有 scheduler、worker/deque 与 Task 3 control plane，定义 Store-free `GcWorkerPool` 生命周期和 packet ownership。
- 写 worker packet/reuse/park/termination RED tests；不改 active GC scheduler。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–6 段、父规格与实施计划；Task 6 commit 已落地，Task 4 Miri protocol 仍受 180 秒约束保留，当前从 Task 7 RED 开始。

## DriftCheckDraft

- Scope：Task 6 在 `managed-heap-v2` 下新增 `ManagedAllocator`/NLAB/page metadata；active runtime allocator、ZGC page owner 与 collector 不变。
- Compatibility：`ObjectRef` 只有 heap-relative offset；NLAB fast path 不取得 allocator mutex，object map/mark bitmap 在 page 创建时预分配，large/humongous 仅从 contiguous page range 分配。
- Retirement：micro allocator 的“counters unavailable”占位输出已替换为真实 30-sample measurement；无旧 micro fallback。V2 allocator 仍由 feature/benchmark crate 唯一触达，Task 15 才切换 active owner。
- Decision：Task 6 GREEN 并提交；Task 4 Miri evidence 仍为 `needs-verification`，继续 Task 7。
