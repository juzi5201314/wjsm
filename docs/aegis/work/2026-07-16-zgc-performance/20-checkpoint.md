...
Task 8：生成 feature-gated shared memory64 backend/support ABI，并保持默认 memory32 backend 完整不变。
...
- 读取 backend module/compiler helper/support artifact 的 current ABI，确认 Task 0 engine config 和 Task 5 V2 entry layout。
- 写 heap memory64 WAT/import RED tests；不改 default backend memory32 path。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–7 段、父规格与实施计划；Task 7 commit 已落地，TSan/Miri evidence 受 180 秒约束保留，当前从 Task 8 RED 开始。

## DriftCheckDraft

- Scope：Task 7 只在 `managed-heap-v2` 下新增 `runtime_gc/worker`；packet 不含 Store/Caller/WasmEnv，active scheduler 不读取 worker pool。
- Compatibility：packet slab 固定容量、warmup 后不增长；shutdown 先停止 admission、以 `inflight` drain、最后 join；无丢包关闭分支或 active scheduler fallback。
- Retirement：V2 worker 为 feature-private 新 owner，Task 15 单点切换时才退休 active scheduler；无双 active ownership。
- Decision：Task 7 core GREEN 并提交；TSan/Miri 为 `needs-verification`（已记录精确环境边界），继续 Task 8。
