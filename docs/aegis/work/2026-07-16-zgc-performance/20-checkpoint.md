...
- 读取 `runtime_gc/heap_access.rs`、`runtime_heap.rs`、`wasm_env.rs` 与所有动态对象 host import，映射 Task 8 new host ABI。
- 写 V2 object/array/property/runtime-string/proxy/collection RED test；不改 default host memory32 path。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–8 段、父规格与实施计划；Task 8 commit 已落地，TSan/Miri evidence 受 180 秒约束保留，当前从 Task 9 RED 开始。

## DriftCheckDraft

- Scope：Task 8 在 backend/support ABI 下新增 V2 `__heap_memory`/i64 cursor globals 和 dynamic helper routing；runtime host 尚未切换，正由 Task 9 实现。
- Compatibility：default feature 保持 memory32 + static helper WAT 断言；V2 feature 有独立 memory64/atomic/dynamic-host assertions，legacy static WAT assertions不在 V2 语义下复用。
- Retirement：V2 compiler helper directory 为 dynamic support ABI 的唯一 caller mapping；legacy inline helpers继续只服务 default backend，Task 15 才删除旧 active path。
- Decision：Task 8 GREEN 并提交；Task 7 TSan/Miri 为 `needs-verification`，继续 Task 9。
