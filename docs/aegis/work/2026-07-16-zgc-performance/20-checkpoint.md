...
- 从 `runtime_gc/g1/*` 与 `MarkSweepV2` 的 RootSnapshot/handle quarantine/page sweep 合同开始 Task 11：迁移 G1 policy 到 V2 底座。
- 保持 Task 10 的 `mark_sweep_v2`、`heap_access_v2` 和 default runtime unit gates 同时通过。

## ResumeStateHint

恢复时先读本文件、`90-evidence.md` 的 Task 0–10 段、父规格与实施计划；Task 10 已实现并完成验证，下一项为 Task 11。

## DriftCheckDraft

- Scope：Task 10 新增 feature-gated `MarkSweepV2`，以 `ManagedHeap<SharedHeapMemory>`、`HandleTableV2` 和 immutable `RootSnapshot` 完成 mark/retire/sweep；OOM 仅在 `OutOfPages` 时触发 full collection 后重试。
- Compatibility：active default collector 未切换；V2 user module 到 legacy startup primordials 的 hybrid read bridge 保留至 Task 15。V2 dynamic object/array/proxy/descriptor/collection result 始终以 atomic handle table 判定 owner。
- Retirement：mark-sweep V2 不读取 `GcContext`、legacy `obj_table` 或 memory32 raw pointer；legacy collector 仅为 default active path 保留，Task 15/26 清除。
- Decision：Task 10 GREEN，进入 Task 11。
