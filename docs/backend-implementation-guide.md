...
`wjsm build --format native-executable` 把预链 `wjsm-exec` stub、canonical `.wjsm` 与当前宿主编译的 `NativeObject` 打成真实 ELF/PE。打包失败不创建或覆盖目标文件。runtime 私有 relocatable object/image 不得单独包装成伪 executable。合同见 ADR 0016。
